...
def load_HDFS(log_file, label_file=None, window='session', train_ratio=0.5, split_type='sequential', save_csv=False, window_size=0):
    """ Load HDFS structured log into train and Source Logs data
    ...
    ---------
        ...
        train_ratio: float, the ratio of training data for train/Source Logs split.
    ...
    if log_file.endswith('.npz'):
        ...
        if label_file:
            ...
            # Split train and Source Logs data
            (x_train, y_train,train_bid), (x_test, y_test,test_bid),x_train_id,x_test_id = _split_data_blockid(data_df['EventSequence'].values,
                data_df['Label'].values, train_ratio, split_type,data_df['BlockId'])
        ...
        if label_file is None:
            ...
            print('Total: {} instances, train: {} instances, Source Logs: {} instances'.format(
    ...
    return (x_train, y_train,train_bid), (x_test, y_test,test_bid),x_train_id,x_test_id
...
def _split_data_blockid(x_data, y_data=None, train_ratio=0, split_type='uniform',blockid_data=None):
    # Initialize block id array
    block_id = blockid_data # Use the given blockid_data array
    if split_type == 'uniform' and y_data is not None:
        pos_idx = y_data > 0
        x_pos = x_data[pos_idx]
        y_pos = y_data[pos_idx]
        block_pos = block_id[pos_idx] # Get the block id for positive samples
        x_neg = x_data[~pos_idx]
        y_neg = y_data[~pos_idx]
        block_neg = block_id[~pos_idx] # Get the block id for negative samples
        train_pos = int(train_ratio * x_pos.shape[0])
        train_neg = int(train_ratio * x_neg.shape[0])
        x_train = np.hstack([x_pos[0:train_pos], x_neg[0:train_neg]])
        y_train = np.hstack([y_pos[0:train_pos], y_neg[0:train_neg]])
        block_train = np.hstack([block_pos[0:train_pos], block_neg[0:train_neg]]) # Get the block id for training samples
        x_test = np.hstack([x_pos[train_pos:], x_neg[train_neg:]])
        y_test = np.hstack([y_pos[train_pos:], y_neg[train_neg:]])
        block_test = np.hstack([block_pos[train_pos:], block_neg[train_neg:]]) # Get the block id for testing samples
    elif split_type == 'sequential':
        num_train = int(train_ratio * x_data.shape[0])
        x_train = x_data[0:num_train]
        x_test = x_data[num_train:]
        block_train = block_id[0:num_train] # Get the block id for training samples
        block_test = block_id[num_train:] # Get the block id for testing samples
        if y_data is None:
            y_train = None
            y_test = None
        else:
            y_train = y_data[0:num_train]
            y_test = y_data[num_train:]
    # Create a dataframe of block id of x_test data
    xbid_test = pd.DataFrame({'block_id': block_test, 'x_test': x_test})
    # Random shuffle
    indexes_train = shuffle(np.arange(x_train.shape[0])) # Create an indexes array for the training set
    indexes_test = shuffle(np.arange(x_test.shape[0])) # Create an indexes array for the testing set
    x_train = x_train[indexes_train] # Shuffle the x_train array
    if y_train is not None:
        y_train = y_train[indexes_train] # Shuffle the y_train array
    block_train = block_train[indexes_train] # Shuffle the block_train array
    # Create a dataframe of block id of x_train data
    xbid_train = pd.DataFrame({'block_id': block_train, 'x_train': x_train}) # Use the shuffled arrays to create a dataframe
    # Shuffle the dataframe of block id of x_test data
    xbid_test = xbid_test.iloc[indexes_test] # Use the iloc method to shuffle the dataframe
    return (x_train, y_train, block_train), (x_test, y_test, block_test), xbid_train, xbid_test
